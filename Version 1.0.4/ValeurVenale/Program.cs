                ...
                if(Country[i] != null && Prices[i] > 3500 && (Types[i] == 1 || Types[i] == 3))
                {
                    if(LivingSpaceSurface[i] != 0)
                    {
                        ...
                        if (Tc != 0)
                        {
                            ...
                            if(Conditions[i] == "OldRefurbished")
                            {
                                UG = 0.04 * VN; //abia e renovata casa deci uzurile sunt ambele diminuata , uzura generala la jumatate pentru ca timpul si-a pus aprenta 
                                UT = 0;
                            }
                            if(Conditions[i] == "OldRefurbished" && Types[i] == 3)
                            {
                                UG = 0.08 * VN; // la apartament uzura generala ramane dupa renovare
                            }
                            if(Conditions[i] == "New" && differenceYear < 3)
                            {
                                UG = UT = 0;
                            }
                            if (Types[i] == 1)
                            ...
                            {
                                //daca vorbim de apartamente uzura este diferita.
                                if (Conditions[i] == "DescConditionGood")
                                {
                                    VGlob = computeValeurGloballe(UT, UG);
                                }
                                ...
                                {
                                    if (Conditions[i] == "Unknown")//inseamna ca e rau si omul nu vrea sa ne spuna
                                    {
                                        VGlob = 2 * computeValeurGloballe(UT, UG);
                                    }
                                    else
                                    {
                                        if (Conditions[i] == "New")//totul este nou deci nu are uzura si are un plus de noutate 
                                        {
                                            UG = UT = 0;
                                            VGlob = computeValeurGloballe(UG, UT);
                                        }
                                        else
                                        {
                                            VGlob = 0.02 * VN;
